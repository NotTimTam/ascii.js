    ...
    if (showValue) {
      const valueMesh = PixelMesh.fromString(
        " " +
          String(value).padEnd(
            Math.max(
              String(max - max / step + (step < 1 ? step : 0)).length - 1,
              1
            ),
            " "
          )
...
class Menu extends GameObject {
  ...
  constructor(scene, config) {
    ...
    scene.inputManager.addEventListener("keydown", this.__onKeyDown.bind(this));
  ...
  __onClick(event) {
    ...
    if (this.focused && !event.targets.includes(this.id)) return this.blur();
  ...
  get renderable() {
    const {
      ...
      width,
      height,
      availableContentSpace: [availableContentWidth, availableContentHeight],
