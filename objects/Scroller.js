...
class Scroller extends UIObject {
  ...
  __handleMouse(event) {
    ...
    if (verticalScrollbar && scrollerX >= this.width - 1) {
      ...
      this.scrollY =
        ((newThumbY - thumbSize / 2) * (this.spans.down - this.spans.up)) /
    ...
    if (horizontalScrollbar && scrollerY >= this.height - 1) {
      const thumbSize = Math.round(
        horizontalScrollbarLength *
          (this.viewportSize[0] / (this.spans.right - this.spans.left))
      );
      ...
      this.scrollX =
        ((newThumbX - thumbSize / 2) * (this.spans.right - this.spans.left)) /
  ...
  get spans() {
    ...
    for (let { x, y, renderable } of this.children) {
      ...
      const { width, height } =
        renderable instanceof PixelMesh ? renderable : { width: 1, height: 1 };
  ...
  get activeScrollbars() {
    ...
    return {
      horizontalScrollbar: spans.right - spans.left > width - borderWidth * 2,
  ...
  get renderable() {
    ...
    const [verticalThumbLength, horizontalThumbLength] = [
      Math.max(
        1,
        Math.round(verticalScrollbarLength * (vH / (spans.down - spans.up)))
      ),
      Math.max(
        1,
        Math.round(
          horizontalScrollbarLength * (vW / (spans.right - spans.left))
        )
      ),
    ];
    ...
    for (const go of children) {
      ...
      if (renderable instanceof Pixel) {
        ...
        for (let pixelY = 0; pixelY < renderable.data.length; pixelY++) {
