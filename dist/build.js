...
var j = Math.pow,
	G = (c, e, t) =>
		e in c
			...
			: (c[e] = t),
	x = (c, e) => {
		for (var t in e || (e = {})) ce.call(e, t) && G(c, t, e[t]);
		if (q) for (var t of q(e)) de.call(e, t) && G(c, t, e[t]);
		return c;
	},
	C = (c, e) => he(c, le(e));
...
const S = (c) =>
		`[${c
			...
			.join(", ")}]`,
	M = (c) => c && typeof c == "object" && !(c instanceof Array),
	Q = (c, e, t = !1) => {
		const r = c.split(" ");
		let i = [],
			n = "";
...
let B = T;
const z = (c, e, t, r, i, n, s, o) =>
		c < i + s && c + t > i && e < n + o && e + r > n,
	L = (c, e, t) => Math.max(e, Math.min(c, t)),
	X = (c) => c * (180 / Math.PI),
	R = (c) => c * (Math.PI / 180),
	ue = (c, e) => Math.floor(Math.random() * (e - c + 1)) + c,
	fe = (c) => {
		if (c === 0 || c === 1) return 1;
		for (let e = c - 1; e >= 1; e--) c *= e;
		return c;
	},
	me = (c, e) => [e * Math.cos(R(c)), e * Math.sin(R(c))],
	pe = (c, e) => [X(Math.atan2(e, c)), Math.sqrt(j(c, 2) + j(e, 2))],
	ge = (c, e, t, r) => X(Math.atan2(r - e, t - c)),
	be = (c, e, t, r) => Math.sqrt(j(t - c, 2) + j(r - e, 2)),
...
class A extends ee {
	...
	constructor(e, t = 0, r = 0, i) {
		...
		(this.__rawX = t),
			(this.__rawY = r),
			(this.__rawVisible = !0),
			(this.__rawRenderable = new y({ value: "#", color: "magenta" })),
			(this.behaviors = []),
			i && (this.layer = i);
	}
	/**
...
const p = class p {
		...
		return r === "standard"
			? {
					axes: p.applyDeadzone(
						Object.fromEntries(
							e
								.filter((i, n) => p.standardAxesMap[n])
								.map((i, n) => [p.standardAxesMap[n], i])
						)
					),
					buttons: Object.fromEntries(
						t
							.filter((i, n) => p.standardButtonMap[n])
							.map((i, n) => [p.standardButtonMap[n], i])
					),
			  }
	...
	get asXB() {
		...
		return {
			axes: Object.fromEntries(
				e
					.filter((r, i) => p.standardAxesMap[i])
					.map((r, i) => [p.standardAxesMap[i], r])
			),
			buttons: Object.fromEntries(
				t
					.filter((r, i) => p.xbButtonMap[i])
					.map((r, i) => [p.xbButtonMap[i], r])
			),
		};
	}
	/**
	...
	get asPS() {
		...
		return {
			...
			buttons: Object.fromEntries(
				t
					.filter((r, i) => p.psButtonMap[i])
					.map((r, i) => [p.psButtonMap[i], r])
			),
		};
	}
	/**
	...
	get asNS() {
		...
		return {
			...
			buttons: Object.fromEntries(
				t
					.filter((r, i) => p.nsButtonMap[i])
					.map((r, i) => [p.nsButtonMap[i], r])
			),
		};
	}
	/**
...
m(p, "gamepadButtonIntervals", 100),
	m(p, "deadzoneThreshold", 0.1),
	m(p, "xbButtonMap", [
		"a",
		"b",
		"x",
...
class ve {
	...
	__onMouseMove(e) {
		const { clientX: t, clientY: r, movementX: i, movementY: n } = e,
			{
				scene: {
					...
					runtime: {
						renderer: { width: l, height: a, element: d },
					},
				},
			} = this,
			{ x: u, y: f, width: _, height: b } = d.getBoundingClientRect(),
			[E, k] = [t - u, r - f],
			[O, v] = [E / _, k / b];
		if (this.hasPointerLock) this.mouse.velocity = [i, n];
			...
			(this.mouse.velocity = [i, n]),
				...
				(this.mouse.x = L(Math.floor(O * l), 0, l)),
				(this.mouse.y = L(Math.floor(v * a), 0, a)),
				(this.mouse.onLayer = {});
			for (const ie of h) {
				const {
					label: ne,
	...
	__triggerEvents(e, t) {
		...
		for (const r of this.__eventListeners[e]) r(t);
	}
	/**
	...
	__onEvent(e) {
		if (e instanceof MouseEvent || e instanceof WheelEvent) {
			...
			this.__triggerEvents(t, x({ type: t }, this.mouse)),
				this.__triggerEvents("all", x({ type: t }, this.mouse)),
			...
			this.__triggerEvents(t, x({ type: t }, this.keyboard)),
				this.__triggerEvents("all", x({ type: t }, this.keyboard));
			...
			this.__triggerEvents(t, { type: t, gamepads: this.gamepads }),
	...
	get frame() {
		...
		for (const l of this.gameObjects.filter(({ visible: a }) => a)) {
			...
			if (a) {
				...
				if (a instanceof y) o(a, d, u);
				else if (a instanceof g) {
					if (!e.isOnScreen(d, u, a.width, a.height, r, i)) continue;
...
class xe {
		...
		for (const o of n) {
			...
			for (const l of h) {
				...
				if (a instanceof y && d === e && u === t)
					...
					a instanceof g &&
					z(e, t, 1, 1, d, u, a.width, a.height)
				) {
	...
	__createLayers(e = []) {
		(this.layers = []),
			e.includes("system") || new Z(this, { label: "system" });
		for (const t of e) {
...
class H {
	...
	constructor(e, t) {
		...
		(this.label = r),
			(this.camera = new ye(this)),
			(this.layerManager = new xe(this, i)),
			(this.inputManager = new ve(this)),
	...
	static validateConfig(e) {
		...
		if (e.scaling) {
			...
			if (!t.includes(e.scaling))
				throw new Error(
					`Invalid scaling value provided, must be one of: ${S(t)}`
				);
		}
		if (e.renderMode) {
			...
			if (!t.includes(e.renderMode))
				throw new Error(
					`Provided render mode is invalid. Must be of type: ${S(t)}`
				);
		}
		if (!e.canvas)
			throw new Error(
					...
					for (let h = 0; h < this.width; h++)
						for (let l = 0; l < this.height; l++) {
							...
							b &&
								...
								s.closePath()),
								s.beginPath(),
								(s.font = `${_ || "normal"} ${r} monospace`),
								(s.fillStyle = f || "#FFFFFF"),
								s.fillText(u, h * i, l * n),
								s.closePath();
						}
				}
			}
...
class Me extends D {
	...
	get renderable() {
		const {
			...
			s = g.fromString(n ? Q(i, t, !0) : i.slice(0, t));
...
class Ce extends D {
	...
	__positionByMouse(e) {
		...
		(t -= P.borderWidth + P.horizontalSpacing),
	...
	get sliderWidth() {
		...
		return e <= 10
			? Math.round(e)
			: L(e, 10, this.menu.runtime.renderer.width / 4);
	}
	get renderable() {
		const {
				menu: { index: e },
		...
		const f = a - 1,
			_ = Math.floor(((r - i) / (n - i)) * f),
			b = new y({
				value: "─",
				color: d ? "white" : "grey",
			}),
			[E, k] = [new Array(_).fill(b), new Array(f - _).fill(b)],
			O = new g({
				data: [
					...E,
...
const w = class w extends A {
	...
	get availableContentSpace() {
		...
		return [e - w.horizontalSpacing * 2 - w.borderWidth * 2, t];
	}
	/**
	...
	get width() {
		...
		return (
			...
			(this.border ? w.horizontalSpacing * 2 : 0) +
			(this.border ? w.borderWidth * 2 : 0)
		);
	}
	/**
	...
	get height() {
		...
		return Math.round(e + (this.border ? w.borderWidth * 2 : 0));
	}
	/**
	...
	get currentContentSpace() {
		...
		return this.border
			? [e - w.borderWidth * 2, t - w.borderWidth * 2]
			: [e, t];
	}
	/**
	 * Get the screen-space available to the content of the menu.
	...
	get renderable() {
		...
		if (e) {
			const o = g.fromString(e.slice(0, this.availableContentSpace[0]));
			...
			for (let a = 0; a < h.length; a++)
				s[0] || (s[0] = []),
					(s[0][a + l + (this.border ? w.horizontalSpacing : 0)] =
						h[a]);
		}
		return new g({ data: s });
	}
...
class U extends A {
	...
	constructor(e, t) {
		...
		if (
			(super(e, r, i, d),
			a && (typeof a != "number" || !Number.isInteger(a) || a < 1))
		)
			throw new TypeError(
	...
	get renderable() {
		...
		return U.asPixelMesh(t, r, e, i, n, s);
	}
	/**
	...
	__tryToMoveToPosition(t, r) {
		...
		if (d || u) {
			const b = i.y - h,
				E = b + s - 1;
			for (let k = b; k <= E; k++) {
				const O = d ? t + n - 1 - o : t - o,
					v = l.solidAtPosition(O, k);
				if (v && v.gameObject !== i) {
					a = !1;
					break;
				}
			...
			const b = i.x - o,
				E = b + n - 1;
			for (let k = b; k <= E; k++) {
				const O = f ? r + s - 1 - h : r - h,
					v = l.solidAtPosition(k, O);
				if (v && v.gameObject !== i) {
					a = !1;
					break;
				}
