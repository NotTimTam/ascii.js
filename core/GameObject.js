...
class GameObject extends Core {
  ...
  constructor(scene, config) {
    if (!isPlainObject(config))
      throw new TypeError(
        "Expected a plain object for GameObject constructor config parameter."
      );
  ...
  set layer(label) {
    ...
    if (label) {
      ...
      if (!layer) throw new Error(`No layer exists with label "${label}"`);
  ...
  get positionInParent() {
    ...
    if (
      ...
      throw new Error('GameObject\'s parent has no "getChildPosition" method.');
  ...
  set parent(gameObject) {
    if (gameObject) {
      ...
      gameObject.children = gameObject.children.filter((gA) => gA !== this);
    }
  }
