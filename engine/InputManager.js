...
class GamepadUIObject {
   ...
   */
  static applyDeadzone(values, threshold = GamepadUIObject.deadzoneThreshold) {
    for (const [name, value] of Object.entries(values)) {
      ...
      values[name] = (value - Math.sign(value) * threshold) / (1 - threshold);
    }
    ...
    if (mapping === "standard")
      return {
        axes: GamepadUIObject.applyDeadzone(
          Object.fromEntries(
            axes
              .filter((_, index) => GamepadUIObject.standardAxesMap[index])
              .map((axis, index) => [
        ...
        buttons: Object.fromEntries(
          buttons
            .filter((axis, index) => GamepadUIObject.standardButtonMap[index])
            .map((axis, index) => [
  ...
  get asXB() {
    ...
    return {
      axes: Object.fromEntries(
        axes
          .filter((axis, index) => GamepadUIObject.standardAxesMap[index])
          .map((axis, index) => [GamepadUIObject.standardAxesMap[index], axis])
      ),
      buttons: Object.fromEntries(
        buttons
          .filter((axis, index) => GamepadUIObject.xbButtonMap[index])
          .map((axis, index) => [GamepadUIObject.xbButtonMap[index], axis])
      ),
    };
  }
    ...
    return {
      ...
      buttons: Object.fromEntries(
        buttons
          ...
          .map((axis, index) => [GamepadUIObject.psButtonMap[index], axis])
      ),
    };
  }
    ...
    return {
      ...
      buttons: Object.fromEntries(
        buttons
          ...
          .map((axis, index) => [GamepadUIObject.nsButtonMap[index], axis])
      ),
    };
  }
...
class InputManager {
  ...
  __onMouseMove(event) {
    ...
    const {
      scene: {
        ...
        layerManager: { layers },
        runtime: {
          renderer: { width: characterWidth, height: characterHeight, element },
        },
      },
    } = this;
  ...
  __triggerEvents(type, data, browserEvent) {
    ...
    data.preventBrowserDefault = browserEvent.preventDefault.bind(browserEvent);
    ...
    if (InputManager.mouseEvents.includes(type)) {
      ...
      if (targets && targets.length !== 0) {
        ...
        this.__triggerUIObjectEvents(target.id, type, data, browserEvent);
      } else if (type === "click") {
  ...
  __onEvent(event) {
    if (event instanceof MouseEvent || event instanceof WheelEvent) {
      ...
      [...this.mouse.targets]
        ...
        .forEach((uIObjectId) =>
          this.__triggerEvents("mouseenter", { type, ...this.mouse }, event)
        );
      ...
      this.__triggerEvents(type, { type, gamepads: this.gamepads }, event); // Trigger the specific event type that fired.
    }
  }
  ...
  addEventListener(type, listener) {
    ...
    if (typeof listener !== "function")
      throw new TypeError('Expected a function for parameter "listener".');
  ...
  __onCreated() {
    ...
    this.__addCanvasEventListener("gamepaddisconnected", this.__eventHandler);
  ...
  __unLoad() {
    ...
    this.__removeCanvasEventListener("gamepadconnected", this.__eventHandler);
  ...
  __onTick() {
    ...
    if (
      ...
      const canTriggerButtonEvent =
        timeSinceLastButtonEvent > GamepadUIObject.gamepadButtonIntervals;
