...
class InputManager {
	...
	/**
	 * Get whether or not the pointer is currently captured.
	 */
	get pointerCaptured() {
		return Boolean(this.mouse.capturer);
	}

	/**
	 * Lock pointer events to a UIObject temporarily.
	 * @param {UIObject} uiObject The UIObject capturing the pointer.
	 * @param {?string|number} mode A string or number indicating the context in which the pointer was captured. Useful for `UIObject` logic in future events.
	 */
	capturePointer(uiObject, mode) {
		if (this.pointerCaptured)
			throw new Error(
				"Pointer has already been captured. Call the InputManager's releasePointer() method to release it."
			);

		if (!(uiObject instanceof UIObject))
			throw new TypeError("Only UIObject's can capture the pointer.");

		if (mode != null) {
			if (typeof mode !== "number" && typeof mode !== "string")
				throw new TypeError(
					"InputManager.capturePointer mode argument must be of type string or number."
				);
			this.mouse.captureMode = mode;
		}
	...
	/**
	 * Release pointer events from the current UIObject capturer.
	 */
	releasePointer() {
	...
	__onMouseUp(event) {
		...
		if (this.pointerCaptured) this.releasePointer();
	...
	__triggerEvents(type, data, browserEvent) {
		...
		if (InputManager.mouseEvents.includes(type)) {
			const targets = this.pointerCaptured // If the pointer has been captured, we only trigger events for the UIObject that has captured it.
				? [this.mouse.capturer]
				: data.targets &&
				  data.targets
						.map((targetId) =>
							this.scene.findGameObjectById(targetId)
						) // Get all target instances.
						.filter(
							(target) => target && target instanceof UIObject
						) // Only UIObject instances are considered targets for UIObject events..
			...
			if (targets && targets.length !== 0) {
				...
				// If the pointer has not been captured, we add a method for capturing it.
				if (!this.pointerCaptured)
					/**
					 * Capture the pointer for `event.target`.
					 * @param {?string|number} mode A string or number indicating the context in which the pointer was captured. Useful for `UIObject` logic in future events.
					 */
					data.capturePointer = (mode) =>
						this.capturePointer(target, mode);
				else data.releasePointer = this.releasePointer;
		...
		// Trigger object specific events for the current focused UIObject. This does not include mouse events as they are handled regardless of focus status.
	 ...
	 */
	__addCanvasEventListener(type, handler, options = undefined) {
		if (!this.__eventListeners[type]) this.__eventListeners[type] = [];
		this.scene.runtime.renderer.element.addEventListener(
			type,
			handler,
			options
		);
	 ...
	 * @param {EventListenerOptions} options Event listener options passthrough.
	 */
	__removeCanvasEventListener(type, handler, options = undefined) {
		delete this.__eventListeners[type];
		this.scene.runtime.renderer.element.removeEventListener(
			type,
			handler,
			options
		);
